/// From:
/// https://github.com/nico1645/zig-stm32l476/blob/main/src/umm_allocator.zig
...
pub fn UmmAllocator(comptime config: Config) type {
	return struct {
		...
		pub fn init(buf: []u8) !Self {
			...
			if (self.heap.len > std.math.maxInt(u16))
		...
		fn alloc(ctx: *anyopaque, len: usize, alignment: std.mem.Alignment, ret_addr: usize) ?[*]u8 {
			_ = ret_addr;
			const log2_align: u8 = @intCast(alignment.toByteUnits());
			...
			loop: while (current != Block.first) {
				...
				if (config.debug_logging)
					logger.debug("Looking at block {s} size {}", .{ @tagName(current), blocks_count });
			...
			if (config.debug_logging)
				logger.debug("Found {any} with size {any}", .{ best_block, best_size });

			const best_block_storage = best_block.get_storage(self);
			if (best_size == target_blocks_count) {
				if (config.debug_logging)
					logger.debug("Allocating {any} blocks starting at {any} - exact", .{ target_blocks_count, best_block });
				best_block_storage.disonnect_from_freelist(self);
			} else {
				if (config.debug_logging)
					logger.debug("Allocating {any} blocks starting at {any} - splitting", .{ target_blocks_count, best_block });
		...
		fn free(ctx: *anyopaque, buf: []u8, alignment: std.mem.Alignment, ret_addr: usize) void {
			...
			if (config.debug_logging)
				logger.debug("Freeing {any}", .{block});
		...
		fn dump_heap(self: *const Self) void {
			...
			while (true) {
				...
				if (config.debug_logging)
					logger.debug("{any} = {any} size: {any}", .{ current, current.get_storage(self), block_size });
				current = next_block;
